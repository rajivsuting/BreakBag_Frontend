...
const AddExclusion = ({ isOpen, onClose, getAlldata }) => {
  ...
  const { title, description } = exclusiondata;

  // Handle form input changes for dynamic descriptions
  const handleChange = (e) => {
    const { name, value } = e.target;
    setExclusiondata({ ...exclusiondata, [name]: value });
  };
...
console.log(exclusiondata)
    try {
      ...
      const response = await axios.post(
        `${serverUrl}/api/exclusion/exclusions/create`,
        exclusiondata
      );
      ...
      setExclusiondata({ title: "", description: "" });
      onClose();
    } catch (error) {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div className="flex justify-center">
            <Button className="bg-main" type="submit" disabled={isLoading}>
              {isLoading ? "Adding Exclusion..." : "Add Exclusion"}
            </Button>
          </div>
        </form>
      </div>
